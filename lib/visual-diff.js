...
class VisualDiff {
  ...
  highlightConflicts(text, conflicts) {
    ...
    // Sort conflicts by line number (descending) to process from bottom up
    const sortedConflicts = [...conflicts].sort((a, b) => b.line - a.line);

    // Insert conflict markers
    for (const conflict of sortedConflicts) {
      const lineIndex = conflict.line - 1; // Convert to 0-based

      if (lineIndex >= 0 && lineIndex < lines.length) {
        // Replace the conflicting line with conflict markers
        const markers = [
          '<<<<<<< LOCAL',
        ...
        lines.splice(lineIndex, 1, ...markers);
      }
    }
