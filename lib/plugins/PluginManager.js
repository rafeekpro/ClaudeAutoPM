...
class PluginManager extends EventEmitter {
  ...
  async _pathExists(p) {
    try {
      ...
      if (error.code !== 'ENOENT') {
        // e.g. EACCES: the path may exist but be unreadable — surface when debugging
  ...
  async installAgents(plugin, pluginPath) {
    ...
    if (!metadata.category) {
      ...
      // keying install:skip on per-resource `name` are not misled
  ...
  async uninstallPlugin(pluginName) {
    ...
    try {
      ...
      // Remove agents (same category guard as installAgents — without a
      // category the path would be .../agents/undefined)
      if (metadata.agents && metadata.agents.length > 0 && metadata.category) {
        const targetDir = path.join(this.options.agentDir, metadata.category);
