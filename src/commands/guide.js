  ...
  async configureGitHub() {
    ...
    const answers = await inquirer.prompt([
      {
        ...
        default: `${path.basename(process.cwd()).replace(/\s+/g, '-')}/${this.projectName || 'my-project'}`,
        validate: (input) => {
    ...
    if (answers.githubToken) {
      try {
        ...
        const checkCommand = `gh repo view ${answers.githubRepo} --json name 2>/dev/null`;
        const env = { ...process.env, GITHUB_TOKEN: answers.githubToken };

        try {
          execSync(checkCommand, { env, stdio: 'pipe' });
          console.log(chalk.green(`\n✓ Repository ${answers.githubRepo} exists and is accessible`));
        } catch (error) {
          // Repo doesn't exist, offer to create it
          ...
          if (createRepo) {
            try {
              ...
              execSync(createCommand, { env, stdio: 'pipe' });
              ...
              console.log(chalk.yellow('\n⚠️  Could not create repository automatically'));
              console.log(chalk.gray('  Create it manually at: ') + chalk.cyan('https://github.com/new'));
            }
          }
        }
      } catch (error) {
  ...
  async configureAzure() {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        default: this.projectName || path.basename(process.cwd()),
