    ...
    const { repoName, githubToken } = await inquirer.prompt([
      ...
      },
      {
        ...
        validate: (input) => {
          ...
          // Classic PAT: ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (40 chars after prefix)
          // Fine-grained PAT: github_pat_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (82 chars after prefix)
          // OAuth: gho_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (40 chars after prefix)
          // User-to-server: ghu_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (40 chars after prefix)
          // Server-to-server: ghs_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (40 chars after prefix)
          // Refresh token: ghr_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx (40 chars after prefix)

          const tokenPatterns = {
            'ghp_': { length: 40, name: 'Classic Personal Access Token' },
            'github_pat_': { length: 82, name: 'Fine-grained Personal Access Token' },
            'gho_': { length: 40, name: 'OAuth Access Token' },
            'ghu_': { length: 40, name: 'User-to-Server Token' },
            'ghs_': { length: 40, name: 'Server-to-Server Token' },
            'ghr_': { length: 40, name: 'Refresh Token' }
          };
          ...
          for (const [prefix, info] of Object.entries(tokenPatterns)) {
            if (input.startsWith(prefix)) {
              tokenType = info.name;
              expectedLength = prefix.length + info.length;
              break;
            }
          }
          ...
          if (input.length !== expectedLength) {
            return `Invalid ${tokenType} length. Expected ${expectedLength} characters, got ${input.length}`;
          }

          // Check if the token contains only valid characters (alphanumeric and underscore)
          const tokenBody = input.substring(input.startsWith('github_pat_') ? 'github_pat_'.length : input.indexOf('_') + 1);
          if (tokenType && Object.keys(tokenPatterns).some(prefix => input.startsWith(prefix))) {
            const matchedPrefix = Object.keys(tokenPatterns).find(prefix => input.startsWith(prefix));
            const tokenBodyCorrect = input.substring(matchedPrefix.length);
            if (!/^[A-Za-z0-9_]+$/.test(tokenBodyCorrect)) {
        ...
        try {
          ...
          if (createRepo) {
            try {
              ...
              const createCommand = `gh repo create ${answers.githubRepo} --public --description "${this.projectDescription || 'Project managed with ClaudeAutoPM'}" --confirm`;
              ...
              execSync(createCommand, { env, stdio: 'pipe', encoding: 'utf-8' });
