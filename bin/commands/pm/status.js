...
exports.command = 'pm:status';
exports.describe = 'status';
...
exports.handler = async (argv) => {
  const spinner = createSpinner('Running pm:status...');
  ...
  try {
    ...
    if (isClaudeCode) {
      ...
      console.log('Run: /pm:status for intelligent status reporting');
      return;
    }

    // Run the deterministic script
    const scriptPath = path.join(process.cwd(), '.claude', 'scripts', 'pm/status.sh');
