...
exports.command = 'pm:next';
exports.describe = 'next';
...
exports.handler = async (argv) => {
  const spinner = createSpinner('Running pm:next...');
  ...
  try {
    ...
    if (isClaudeCode) {
      ...
      console.log('Run: /pm:next for intelligent task prioritization');
      return;
    }

    // Run the deterministic script
    const scriptPath = path.join(process.cwd(), '.claude', 'scripts', 'pm/next.sh');
