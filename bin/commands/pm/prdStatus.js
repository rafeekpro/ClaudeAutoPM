...
exports.command = 'pm:prd-status';
exports.describe = 'prd-status';
...
exports.handler = async (argv) => {
  const spinner = createSpinner('Running pm:prd-status...');
  ...
  try {
    ...
    if (isClaudeCode) {
      ...
      console.log('Run: /pm:prd-status for intelligent PRD status');
      return;
    }

    // Run the deterministic script
    const scriptPath = path.join(process.cwd(), '.claude', 'scripts', 'pm/prd-status.sh');
