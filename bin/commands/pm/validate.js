...
exports.handler = async (argv) => {
  const spinner = createSpinner('Running pm:validate...');
  ...
  try {
    ...
    if (isClaudeCode) {
      ...
      console.log('Run: /pm:validate for intelligent validation checks');
      return;
    }

    // Run the deterministic script
    const scriptPath = path.join(process.cwd(), '.claude', 'scripts', 'pm/validate.sh');
